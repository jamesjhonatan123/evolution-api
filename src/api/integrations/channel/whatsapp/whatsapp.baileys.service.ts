...
export class BaileysStartupService extends ChannelStartupService {
  ...
  public async whatsappNumber(data: WhatsAppNumberDto) {
    const jids: {
      ...
      lids: { number: string; jid: string }[]; // Números que parecem ser LIDs
    } = { groups: [], broadcast: [], users: [], lids: [] };
    ...
    data.numbers.forEach((number) => {
      ...
      // Verificar se parece ser um LID pelo formato (número longo sem código de país)
      if (this.isLikelyLidNumber(number)) {
        const lidJid = `${cleanNumber}@lid`;
        this.logger.verbose(`[LID-DETECT] Treating ${number} as LID: ${lidJid}`);
        jids.lids.push({ number, jid: lidJid });
        return;
      }
    ...
    // LIDs detectados pelo formato (números longos sem código de país)
    // Estes são tratados como válidos e enviados diretamente via @lid
    const detectedLids = jids.lids.map(({ jid, number }) => {
      this.logger.verbose(`[LID-DETECT] Adding detected LID to response: ${jid}`);
      return new OnWhatsAppDto(jid, true, number, undefined, 'lid');
    });
    onWhatsapp.push(...detectedLids);
